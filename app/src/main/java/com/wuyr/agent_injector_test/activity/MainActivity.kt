package com.wuyr.agent_injector_test.activity
...
import com.wuyr.agent_injector.AgentInjector
import com.wuyr.agent_injector.AgentInjector.deviceRooted
import com.wuyr.agent_injector.adb.AdbClient
import com.wuyr.agent_injector.adb.AdbWirelessPairing
import com.wuyr.agent_injector.adb.AdbWirelessPortResolver
import com.wuyr.agent_injector.adb.AdbWirelessPortResolver.Companion.resolveAdbPairingPort
import com.wuyr.agent_injector.adb.AdbWirelessPortResolver.Companion.resolveAdbTcpConnectPort
import com.wuyr.agent_injector.adb.AdbWirelessPortResolver.Companion.resolveAdbWirelessConnectPort
import com.wuyr.agent_injector.exception.AdbCommunicationException
...
import com.wuyr.agent_injector_test.BuildConfig
import com.wuyr.agent_injector_test.Drug
import com.wuyr.agent_injector_test.R
import com.wuyr.agent_injector_test.adapter.AppItem
import com.wuyr.agent_injector_test.adapter.AppListAdapter
import com.wuyr.agent_injector_test.databinding.ActivityMainBinding
import com.wuyr.agent_injector_test.log.logE
import java.io.File
...
class MainActivity(override val viewBindingClass: Class<ActivityMainBinding> = ActivityMainBinding::class.java) : BaseActivity<ActivityMainBinding>() {
    ...
    override fun onCreate() {
        binding.apply {
            ...
            closeSelinuxButton.setOnClickListener { closeSelinux() }
    ...
    private var deviceRooted = false
    private var magiskInstalled = false
    ...
    private fun onConnectPortDetected(host: String, port: Int) {
        ...
        runCatching {
            ...
            AdbClient.openShell(host, port).use {
                ...
                selinuxEnabled = it.sendShellCommand("getenforce").contains("Enforcing")
            ...
            connectPort = port
            runOnUiThread {
                // 必须拥有root权限才可以关闭selinux
                if (selinuxEnabled && deviceRooted) {
                    binding.closeSelinuxButton.visibility = View.VISIBLE
                }
    ...
    private fun closeSelinux() {
        threadPool.execute {
            runCatching {
                AdbClient.openShell(connectHost, connectPort).use { adb ->
                    adb.sendShellCommand("su")
                    if (adb.sendShellCommand("id").contains("uid=0(")) {
                        adb.sendShellCommand("setenforce 0")
                        if (adb.sendShellCommand("getenforce").contains("Enforcing")) {
                            runOnUiThread {
                                AlertDialog.Builder(this).setMessage(R.string.failed_to_close_selinux).setPositiveButton(android.R.string.ok, null).show()
                            }
                        } else {
                            runOnUiThread {
                                binding.closeSelinuxButton.visibility = View.GONE
                                Toast.makeText(this, R.string.selinux_closed_successfully_please_do_inject_again, Toast.LENGTH_LONG).show()
                            }
                        }
                    } else {
                        runOnUiThread {
    ...
    private fun doInject(useRoot: Boolean, targetPackageName: String, injectMethodName: String) {
        threadPool.execute {
            runCatching {
                AgentInjector.inject(connectHost, connectPort, packageName, useRoot, targetPackageName, targetPackageName, packageResourcePath, Drug::class.java.name, injectMethodName)
            }.onFailure {
                it.stackTraceToString().logE()
                val message = when (it) {
